...
└── src/
    ├── original/
    │   ├── SRI.java              -> SRI original com o bug de overflow
    │   ├── OBC.java              -> OBC original sem validação
    │   └── SimulacaoFalha.java   -> Reproduz o desastre de 1996
    │
    └── corrigido/
        ├── EstadoCircuito.java      -> Enum com estados do Circuit Breaker
        ├── ObservadorVoo.java       -> Interface Observer
        ├── EstacaoTerrestre.java    -> Observer: estação terrestre
        ├── MonitorSeguranca.java    -> Observer: monitor de alertas
        ├── EstrategiaConversao.java -> Interface Strategy
        ├── ConversaoSegura.java     -> Strategy: conversão com clamping
        ├── ResultadoSRI.java        -> Dado de navegação com flag de validade
        ├── CircuitBreaker.java      -> Circuit Breaker com suporte a Observer
        ├── SRICorrigido.java        -> SRI com Strategy injetada
        ├── OBCCorrigido.java        -> OBC com validação e Circuit Breaker
        └── SimulacaoCorrigida.java  -> Simula a arquitetura proposta
```
